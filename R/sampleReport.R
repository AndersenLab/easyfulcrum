...
collection_distinct <- c("total distinct",
                         pull(collection_counts_1%>%dplyr::distinct(`total number of distinct collections`)%>%dplyr::filter(!is.na(`total number of distinct collections`))))
...
collection_distinct <- data.frame(t(collection_distinct)) %>% 
...
DT::datatable(collection_distinct, colnames = c("","number of collections"), 
...
if(elegans_eval == FALSE) {
  warning("No collections in this project identified C. elegans samples")
}

## ---- fig.height=5, fig.width=10, echo=F, warning = F, message=F, eval=elegans_eval----
#  target_df <- df %>%
#    dplyr::group_by(c_label, species_id) %>%
#    dplyr::mutate(species_count = n()) %>%
#    dplyr::mutate(`C. elegans count` = ifelse(species_id == "Caenorhabditis elegans" & !is.na(species_id), species_count, 0),
...
#                  collection_trail)
#  
#  DT::datatable(elegans_df, rownames = FALSE, filter = "none", options = list(dom = 'ftrp'))

## ---- fig.height=8, fig.width=8, echo=F, warning = F, message=F, eval=elegans_eval----
#  stat_df <- df%>%
#    dplyr::filter(species_id == "Caenorhabditis elegans") %>%
#    dplyr::distinct(project, c_label, species_id, .keep_all = T) %>%
#    dplyr::select(project, c_label, s_label, species_id, proc_ambient_temperature,
...
if(briggsae_eval == FALSE) {
  warning("No collections in this project identified C. briggsae samples")
}

## ---- fig.height=5, fig.width=10, echo=F, warning = F, message=F, eval=briggsae_eval----
target_df <- df %>%
  dplyr::group_by(c_label, species_id) %>%
  dplyr::mutate(species_count = n()) %>%
  dplyr::mutate(`C. elegans count` = ifelse(species_id == "Caenorhabditis elegans" & !is.na(species_id), species_count, 0),
...
DT::datatable(briggsae_df, rownames = FALSE, filter = "none", options = list(dom = 'ftrp'))

## ---- fig.height=8, fig.width=8, echo=F, warning = F, message=F, eval=briggsae_eval----
stat_df <- df%>%
  dplyr::filter(species_id == "Caenorhabditis briggsae") %>%
  dplyr::distinct(project, c_label, species_id, .keep_all = T) %>%
  dplyr::select(project, c_label, s_label, species_id, proc_ambient_temperature,
...
if(tropicalis_eval == FALSE) {
  warning("No collections in this project identified C. tropicalis samples")
}

## ---- fig.height=5, fig.width=10, echo=F, warning = F, message=F, eval=tropicalis_eval----
#  target_df <- df %>%
#    dplyr::group_by(c_label, species_id) %>%
#    dplyr::mutate(species_count = n()) %>%
#    dplyr::mutate(`C. elegans count` = ifelse(species_id == "Caenorhabditis elegans" & !is.na(species_id), species_count, 0),
...
#                  collection_trail)
#  
#  DT::datatable(tropicalis_df, rownames = FALSE, filter = "none", options = list(dom = 'ftrp'))

## ---- fig.height=8, fig.width=8, echo=F, warning = F, message=F, eval=tropicalis_eval----
#  stat_df <- df%>%
#    dplyr::filter(species_id == "Caenorhabditis tropicalis") %>%
#    dplyr::distinct(project, c_label, species_id, .keep_all = T) %>%
#    dplyr::select(project, c_label, s_label, species_id, proc_ambient_temperature,
