...
checkGenotypes <- function(geno_data, fulc_data, return = FALSE) {
  ...
  # missing s_labels
  missing_s_label_genotyping <- data %>% dplyr::filter(flag_missing_s_label_genotyping == TRUE)
  ...
  # dupicated s_labels
  duplicated_s_label_genotyping <- data %>% dplyr::filter(flag_duplicated_s_label_genotyping == TRUE)
  ...
  # its2 check
  its2_genotype_expected <- data %>% dplyr::filter(flag_its2_genotype_expected == TRUE)
  ...
  # species_id check
  species_id_expected <- data %>% dplyr::filter(flag_species_id_expected == TRUE)
  ...
  # unusual target name
  unusual_target_species_name <- data %>% dplyr::filter(flag_unusual_target_species_name == TRUE)
