...
Collection protocol can be found here: [Nematode Collection Protocol](https://docs.google.com/document/d/1aPgPDYobpxzr0BQYgvUsjPWnAGXZmCSQKQ4sDGBXuYM/edit).
...
```{r}
proc_data1_clean <- fixTemperatures(data = proc_fulc1,
...
Details on how to create genotyping google sheets can be found [here](), while further details of the googlesheets4 package (which underlies this function) and how to connect to a personalized google sheet can be found [here](https://googlesheets4.tidyverse.org/articles/articles/read-sheets.html), with details on how to specify the `col_types` specified [here](https://googlesheets4.tidyverse.org/reference/range_read.html#column-specification) if needed.
...
proc_geno1 <- checkGenotypes(geno_data = raw_geno1, fulc_data = anno_fulc1, 
                                  return_geno = TRUE, return_flags = TRUE)
```
